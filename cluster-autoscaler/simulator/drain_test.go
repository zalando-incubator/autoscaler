...
func TestFastGetPodsToMove(t *testing.T) {
	...
	_, blockingPod, err = FastGetPodsToMove(schedulerframework.NewNodeInfo(pod11), true, true, nil, testTime)
	assert.Error(t, err)
	assert.Equal(t, &drain.BlockingPod{Pod: pod11, Reason: drain.NotReplicated}, blockingPod)
}
