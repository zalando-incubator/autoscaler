...
func MarkBeingDeleted(node *apiv1.Node, client kube_client.Interface, cordonNode bool) error {
	return addTaint(node, client, BeingDeletedTaint, apiv1.TaintEffectNoSchedule, cordonNode)
}
...
func CleanBeingDeleted(node *apiv1.Node, client kube_client.Interface) (bool, error) {
	return cleanTaint(node, client, BeingDeletedTaint, false)
}
...
func CleanAllBeingDeleted(nodes []*apiv1.Node, client kube_client.Interface, recorder kube_record.EventRecorder) {
	cleanAllTaints(nodes, client, recorder, BeingDeletedTaint, false)
}
